...
class BasicAgent:
    ...
    def _affected_by_traffic_light(self, lights_list=None, max_distance=None):
        ...
        # Re-validate the last traffic light
        if self._last_traffic_light:
            if self._last_traffic_light.state != carla.TrafficLightState.Red:
                self._last_traffic_light = None
            else:
                # Even if it's still red, check if we have already passed it
                if self._last_traffic_light.id in self._lights_map:
                    trigger_wp = self._lights_map[self._last_traffic_light.id]
                else:
                    trigger_location = get_trafficlight_trigger_location(self._last_traffic_light)
                    trigger_wp = self._map.get_waypoint(trigger_location)
                    self._lights_map[self._last_traffic_light.id] = trigger_wp

                # Check if the vehicle has already passed the stop line of this light
                v_t = ego_vehicle_location - trigger_wp.transform.location
                t_fwd = trigger_wp.transform.get_forward_vector()
                if v_t.x * t_fwd.x + v_t.y * t_fwd.y > 0:
                    self._last_traffic_light = None
                elif not is_within_distance(trigger_wp.transform, self._vehicle.get_transform(), max_distance, [0, 90]):
                    self._last_traffic_light = None
                else:
        ...
        for traffic_light in lights_list:
            ...
            # Check if the vehicle has already passed the stop line
            v_t = ego_vehicle_location - trigger_wp.transform.location
            t_fwd = trigger_wp.transform.get_forward_vector()
            if v_t.x * t_fwd.x + v_t.y * t_fwd.y > 0:
                continue
