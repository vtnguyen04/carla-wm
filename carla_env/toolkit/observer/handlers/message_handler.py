...
class MessageHandler(BaseHandler):
    ...
    def get_observation_space(self) -> Dict:
        return {
            ...
            "dest": spaces.Box(
                low=0, high=1, shape=(self._config.dest_num,), dtype=np.float32
            ),
        }
    ...
    def get_observation(self, env_state: Dict) -> Tuple[Dict, Dict]:
        ...
        neighbors = get_neighbors(
            self._ego, self._world.actor_transforms, is_fov_visible
        )
        actor_actions = self._world.actor_actions
        text = np.zeros(
            (self._config.neighbor_num, self._config.n_commands), dtype=np.float32
        )
