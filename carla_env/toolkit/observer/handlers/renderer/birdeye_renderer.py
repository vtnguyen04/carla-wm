...
class BirdeyeRenderer:
    ...
    def _render_ego_vehicle(self, **env_state):
        ...
        ego_id = self._ego.id
        if ego_id not in self._world_manager.actor_polygons:
            return
        ego_polygon = self._world_manager.actor_polygons[ego_id]
    ...
    def _render_path(
        ...
        # skip rendering obsolete path points (chỉ lọc các điểm ở phía sau xe)
        start = 0
        for i, p in enumerate(path):
            ...
            if np.dot(p_direction, vehicle_direction) < 0:
                ...
                break # Phải ngắt vòng lặp ngay khi gặp điểm phía trước, nếu không đoạn cua gấp (nhỏ hơn 0 độ) sẽ xóa sổ toàn bộ Path tương lai!
