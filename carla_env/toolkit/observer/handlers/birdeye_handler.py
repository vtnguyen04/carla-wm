...
class BirdeyeHandler(BaseHandler):
    ...
    def reset(self, ego: carla.Actor) -> None:
        ...
        # Recreate the renderer if the map was rotated
        if not hasattr(self, '_current_map_name') or self._current_map_name != self._world.carla_map.name:
            self._recreate_renderer()
            self._current_map_name = self._world.carla_map.name

        self._birdeye_render.set_ego(ego)

    def _recreate_renderer(self):
        pixels_per_meter = self._display_size / self._config.obs_range
        pixels_ahead_vehicle = (
            self._config.obs_range / 2 - self._config.ego_offset
        ) * pixels_per_meter
