...
EVAL_SCRIPT="-m torch_wm.rl.eval"
COMMON_PARAMS="--method twister --env.world.carla_port $CARLA_PORT --twister.run.from_checkpoint $CHECKPOINT_PATH"
ADDITIONAL_PARAMS="${@:4}"  # Capture all additional parameters passed to the script
export CUDA_VISIBLE_DEVICES=$GPU_DEVICE
# Unset LD_LIBRARY_PATH to prevent Conda/Shell libs from conflicting with system drivers
unset LD_LIBRARY_PATH
export LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libstdc++.so.6
# Use direct python from venv to ensure LD_PRELOAD is preserved
EVAL_COMMAND=".venv/bin/python $EVAL_SCRIPT $COMMON_PARAMS $ADDITIONAL_PARAMS"
