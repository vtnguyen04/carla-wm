...
class ModularExpert:
    def __init__(self, d_steer, d_acc, target_speed_ms):
        ...
        self.lateral = PIDController(Kp=0.8, Ki=0.01, Kd=0.6, dt=0.05)
        self.prev_steer, self.alpha = 0.0, 1.0
    ...
    def plan(self, ego, world, wpts, current_speed_ms, env_info):
        ...
        if len(wpts) > 1:
            ...
            Ld = max(
                8.0, current_speed_ms * 1.8
            )  #             target_idx = min(len(wpts)-1, 4)
            ...
            target_loc = carla.Location(
                float(wpts[target_idx][0]), float(wpts[target_idx][1]), 0
            )
        ...
        ttc, lead_dist = TTCCalculator.get_ttc_and_distance(
            ego, world, world.get_map(), wpts
        )
        if lead_dist < 0:
            lead_dist = 200.0  # Clear
        ...
        if max_yaw_diff > 10.0:
            curve_penalty = 1.0 - min(
                max_yaw_diff / 90.0, 0.6
            )  # Reduces up to 60% of speed on a 90-deg turn
            target_speed = max(
                base_speed_ms * curve_penalty, 3.5
            )  # Minimum cornering speed 3.5m/s (12.6 km/h)
        else:
        ...
        if lead_dist < 7.0 or (0.0 < ttc < 2.0):  # Priority 1: Instant Stop Zone
            ...
            # 3. Long Distance Approach
                ...
                if current_speed_ms < target_approach_ms:
                    ...
                    desired_acc = 0.0
                    status = f"RED COAST {dist_to_red_light:.1f}m"
        else:
...
def main():
    ...
    print(config)
    action_cfg = model_configs["defaults"].get("env_params", {}).get("action", {})
    # steer_vals = action_cfg.get("discrete_steer", [-0.6, -0.2, 0.0, 0.2, 0.6])
    steer_vals = [-0.6, -0.2, 0.0, 0.2, 0.6]
    steer_str = ",".join([str(x) for x in steer_vals])

    # 1. Setup Env - HIGH FIDELITY COLLECTION MODE
    print("Initializing CARLA Environment...")
    env, env_config = carla_env.create_task(
        "carla_navigation",
        [
            "--env.world.carla_port",
            "2000",
            "--env.params.num_vehicles",
            "15",
            "--env.display.hires",
            "True",
            "--env.display.width",
            ...
            "--env.display.height",
            "720",
            "--env.display.image_size",
            "128",
            "--env.eval",
            "False",
            "--env.action.discrete_steer",
            steer_str,
            "--env.world.fixed_delta_seconds",
            "0.05",
            "--env.world.actor_active_distance",
            "150",
            "--env.observation.camera.attributes.image_size_x",
            "128",
            "--env.observation.camera.attributes.image_size_y",
            "128",
            "--env.observation.camera.shape",
            "128",
            "128",
            "3",
            "--env.observation.camera.attributes.sensor_tick",
            "0.05",
            "--env.observation.spectator.sensor_tick",
            "0.2",
            "--env.display.no_monitor",
            "False",
        ],
    )
    action_spec = env_config.env.action
    d_steer, d_acc = action_spec.discrete_steer, action_spec.discrete_acc
    print("discrete_acc: ", d_acc)
    print("discrete_steer: ", d_steer)
    total_actions = len(d_steer) * len(d_acc)
    env = wrap_env(from_gym.FromGym(env), config)
    env = embodied.BatchEnv([env], parallel=False)

    # SAFE CAPACITY FOR 720p (20k frames)
    replay = embodied.replay.Uniform(32, 20000000, pathlib.Path("./expert_data/replay"))

    print(f"\nEXPERT ONLINE. (Action Space: {total_actions})")
    ...
    try:
        for ep in range(1, 11):
            print(f"🌟 EP {ep} START")
            obs = env.step(
                {"action": np.zeros((1, total_actions)), "reset": np.array([True])}
            )
            ...
            if hasattr(raw_env, "env"):
                raw_env = raw_env.env  # For some nested gym structures
            if hasattr(raw_env, "unwrapped"):
                raw_env = raw_env.unwrapped

            expert = ModularExpert(d_steer, d_acc, 40.0 / 3.6)
            ...
            while not done:
                ...
                for event in events:
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_1:
                            ...
                            print(
                                f"{'⏸️ ĐÃ TẠM DỪNG (Bấm P lần nữa để Đi Tiếp)' if is_paused else '▶️ ĐÃ TIẾP TỤC SIMULATION'}"
                            )
                ...
                if is_paused:
                    ...
                    monitor.clock.tick(60)  # Increase to 60 FPS for smoother UI
                    pygame.display.flip()
                    continue
                ...
                if drive_mode == "AUTOPILOT":
                    action_idx, v_steer, status, lead_d = expert.plan(
                        ego,
                        raw_env._world.carla_world,
                        wpts,
                        get_speed(ego) / 3.6,
                        env_info,
                    )
                    ...
                    if keys[pygame.K_a]:
                        manual_steer = 0.6  # Sang trái
                    elif keys[pygame.K_d]:
                        manual_steer = -0.6  # Sang phải
                    ...
                    if len(wpts) > 0:
                        lead_d = np.linalg.norm(
                            np.array([ego.get_location().x, ego.get_location().y])
                            - np.array(wpts[0][:2])
                        )
                    else:
                        lead_d = 100.0
                ...
                # Step
                obs, info = env.step(
                    {"action": np.expand_dims(one_hot, 0), "reset": np.array([False])}
                )
                ep_reward += obs["reward"][0]
                done = obs["is_last"][0]

                # --- OPTIMIZED DISPLAY (SKIP HIRES) ---
                monitor_obs = {k: v[0] for k, v in obs.items()}
                # Skip expensive hires processing for better FPS

                monitor.sim_time = sim_time

                # Compute metrics efficiently
                ttc, dist_front = TTCCalculator.get_ttc_and_distance(
                    ego, raw_env._world.carla_world, raw_env._world.carla_map
                )
                ego_pos = np.array([ego.get_location().x, ego.get_location().y])
                wpt_dist = (
                    np.linalg.norm(np.array(wpts[0][:2]) - ego_pos)
                    if len(wpts) > 0
                    else 0.0
                )
                ...
                hud_info = {
                    ...
                    "throttle": ego.get_control().throttle,
                    "steer": ego.get_control().steer,
                    "brake": ego.get_control().brake,
                ...
                # Render every step but with optimized pygame settings
                ...
                replay_step = {
                    ...
                    if k in env.obs_space or k in ["reward", "is_first", "is_last"]
                }
                replay_step["action"] = one_hot
                replay.add(replay_step)

                # INCREMENTAL SAVE TO PREVENT LOSS ON OOM
                if total_steps % 100 == 0:
                    print(f"Incremental save at step {total_steps}...")
                    replay.save()
